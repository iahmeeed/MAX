...
local function setDoorState(id, state, lockpick)
	...
	if door then
		...
		if source then
			...
			QBCore.Functions.Notify(source, state == 0 and 'Cannot Unlock This Door' or 'Cannot Lock This Door', 'error')
...
-- Remove lockpick item when failed
RegisterNetEvent('ox_doorlock:removeLockpickItem', function(item)
    local src = source
