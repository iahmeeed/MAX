...
local function pickLock(entity)
    ...
    exports['skillchecks']:startAlphabetGame(10000, 15, function(result)
        success = result
        finished = true
    end)
    ...
    if success then
        ...
        QBCore.Functions.Notify('The Door Opened Successfully', 'success')
    else
        -- ❌ Failed → remove lockpick item
        local item = Config.LockpickItems[1] -- use first item from config
        TriggerServerEvent('ox_doorlock:removeLockpickItem', item)
        QBCore.Functions.Notify('Failed To Open the Door', 'error')
...
-- DOOR CREATION AND UI CODE (unchanged)
local tempData = {}
...
RegisterNUICallback('createDoor', function(data, cb)
    ...
    if data.lockpickDifficulty and not next(data.lockpickDifficulty) then data.lockpickDifficulty = nil end
    if data.groups and not next(data.groups) then data.groups = nil end
...
-- Add target interaction for lockpicking
CreateThread(function()
    ...
    elseif GetResourceState('qtarget'):find('start') then
        target = { qt = true, exp = exports.qtarget }
    end
