...
AddEventHandler('playerDropped', function(reason)
    ...
    QBCore.ShowDrop(GetCurrentResourceName(), ""..GetPlayerName(src).." | Reason : "..reason.."")
    ...
    if QBCore.Player_Buckets[Player.PlayerData.license] then 
...
RegisterNetEvent('QBCore:Server:RemoveItem', function(itemName, amount, slot)
    ...
    if Player then 
        ...
        TriggerEvent("Rc2-log:server:CreateLog", 
            ...
            "Item Removed",
            "red",
            "**"..GetPlayerName(src).."** \n **citizenid : "..cid.."** \n **ID : "..src.."** \n Lost Item : **"..itemName.."** Amount : **"..amount.."**",
            false
        )
    end
end)
...
RegisterNetEvent('QBCore:Server:AddItem', function(itemName, amount, slot, info, reason)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player then 
        if not amount then
            print(''..GetPlayerName(src)..' is cheating ID : '..src..'')
            TriggerEvent("Rc2-log:server:CreateLog", 'fuckers', 'Fuckers Founded', 'red', GetPlayerName(src) .. ' Have Been Marked As Cheater', true)
            DropPlayer(src, 'Ok noob')
            return
        end 
        ...
        TriggerEvent("Rc2-log:server:CreateLog", 
            ...
            "Item Added Event",
            "green",
            "**"..GetPlayerName(src).."** \n **citizenid : "..cid.."** \n **ID : "..src.."** \n Received Item : **"..itemName.."** Amount : **"..amount.."**",
            false
        )
    end
end)

-- Non-Chat Command Calling (ex: Rc2-adminmenu)
