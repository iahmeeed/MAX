...
function QBCore.Functions.CreateLog(name, title, color, message, tagEveryone)
    TriggerEvent('Rc2-log:server:CreateLog', name, title, color, message, tagEveryone)
end

--[[function QBCore.Functions.Ban(source, reason, BanTime, isAuto)
    local src = source
    if src then 
        ...
        MySQL.Async.insert('INSERT INTO bans (name, license, steam, discord, ip, reason, expire, bannedby) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', {
            GetPlayerName(src),
            QBCore.Functions.GetIdentifier(src, 'license'),
            --QBCore.Functions.GetIdentifier(src, 'steam'),
        ...
        TriggerEvent("Rc2-log:server:CreateLog", 'fuckers', 'Player Banned', 'red', string.format('%s %s %s %s was banned by %s for %s', GetPlayerName(src), QBCore.Functions.GetIdentifier(src, 'steam'), QBCore.Functions.GetIdentifier(src, 'discord'), QBCore.Functions.GetIdentifier(src, 'ip'), 'Auto Ban', reason), true)
        if isAuto then 
            TriggerClientEvent('chatMessage', -1, "Auto Ban System ", 'error', ""..GetPlayerName(src).." Enjoy your stay in waqwaq island do not even try to do that again ")
            DropPlayer(src, 'Enjoy your stay in waqwaq island do not even try to do that again')
        else
            TriggerClientEvent('chatMessage', -1, "Jaber Airlines ", 'error', ""..GetPlayerName(src).." Has been banned for : "..reason.." ")
            DropPlayer(src, 'You Have Been Banned For '..reason..'')
        end
        return true
    end
    return false
...
QBCore.Functions.InsertSql = function(wait, query, cb)
	local rtndata = {}
	local waiting = true
...
QBCore.Functions.UpdateSql = function(wait, query, cb)
	local rtndata = {}
	local waiting = true
	exports['oxmysql']:update(query, {}, function(data)
		if cb ~= nil and wait == false then
			cb(data)
		end
		rtndata = data
        ...
        if QBCore.Config.Server.PermissionList[license] then
            if QBCore.Config.Server.PermissionList[license].license == license then
                for _, permLevel in pairs(permission) do
                    if QBCore.Config.Server.PermissionList[license].permission == permLevel or QBCore.Config.Server.PermissionList[license].permission == 'god' then
                        return true
                    end
                end
            end
...
function QBCore.Functions.IsOptin(source)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    local license = QBCore.Functions.GetIdentifier(source, 'license')
    if QBCore.Functions.HasPermission(src, QBCore.Config.Server.Permissions) then
...
function QBCore.Functions.BoxShow(source, isAdd, item, amount)
    if isAdd then 
...
function QBCore.Functions.TimeFormate(Time)
    if Time then 
        local timeTable = os.date('*t', Time)
        return timeTable['day'] .. '/' .. timeTable['month'] .. '/' .. timeTable['year'] .. ' ' .. timeTable['hour'] .. ':' .. timeTable['min']
    else
        return nil
    end
end