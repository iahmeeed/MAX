...
end, { 'god', 'admin', 'operator', 'staff', 'supervisor', 'trusted', 'trial', 'mod', 'support' })
...
end, { 'god', 'admin', 'operator', 'staff', 'supervisor' })
...
QBCore.Commands.Add('car', Lang:t("command.car.help"), {{ name = Lang:t("command.car.params.model.name"), help = Lang:t("command.car.params.model.help") }}, true, function(source, args)
    ...
    TriggerEvent("Rc2-log:server:CreateLog", 
        ...
        "Admin Spawn vehicle",
        "green",
        "**"..GetPlayerName(source) .. "** has spawned a vehicle "..tostring(args[1]),
        false
    )
end, { 'god', 'admin', 'operator', 'staff' })
...
QBCore.Commands.Add('dv', Lang:t("command.dv.help"), {}, false, function(source)
    ...
    TriggerEvent("Rc2-log:server:CreateLog", 
        ...
        "Admin DV vehicle",
        "green",
        "**"..GetPlayerName(source) .. "** has deleted a vehicle",
        false
    )
end, { 'god', 'admin', 'operator', 'staff', 'supervisor', 'trusted', 'trial' })
...
QBCore.Commands.Add('givemoney', Lang:t("command.givemoney.help"), { { name = Lang:t("command.givemoney.params.id.name"), help = Lang:t("command.givemoney.params.id.help") }, { name = Lang:t("command.givemoney.params.moneytype.name"), help = Lang:t("command.givemoney.params.moneytype.help") }, { name = Lang:t("command.givemoney.params.amount.name"), help = Lang:t("command.givemoney.params.amount.help") } }, true, function(source, args)
    ...
    if Player then
        ...
        TriggerEvent("Rc2-log:server:CreateLog", 
            ...
            "Admin Give Money",
            "green",
            "**"..GetPlayerName(source) .. "** has gived money type "..tostring(args[2]).." with amount of $ **"..tonumber(args[3]).."** to **"..GetPlayerName(Player.PlayerData.source).."**",
            false
        )
    else
        TriggerClientEvent('QBCore:Notify', source, Lang:t('error.not_online'), 'error')
...
QBCore.Commands.Add('setjob', Lang:t("command.setjob.help"), { { name = Lang:t("command.setjob.params.id.name"), help = Lang:t("command.setjob.params.id.help") }, { name = Lang:t("command.setjob.params.job.name"), help = Lang:t("command.setjob.params.job.help") }, { name = Lang:t("command.setjob.params.grade.name"), help = Lang:t("command.setjob.params.grade.help") } }, true, function(source, args)
    ...
    if args[2] == nil or args[3] == nil then
        return TriggerClientEvent('QBCore:Notify', source, "!!! Complete the fucking command", 'error')
    end
    if Player then
        -- exports['Rc2-phone']:hireUser(tostring(args[2]), Player.PlayerData.citizenid, tonumber(args[3]))
        Wait(500)
        ...
        TriggerEvent("Rc2-log:server:CreateLog", 
            ...
            "Admin Set Job",
            "green",
            "**"..GetPlayerName(source) .. "** has gived job type : **"..tostring(args[2]).."** grade **"..tonumber(args[3]).."** to **"..GetPlayerName(Player.PlayerData.source).."**",
            false
        )
    else
        TriggerClientEvent('QBCore:Notify', source, Lang:t('error.not_online'), 'error')
...
QBCore.Commands.Add('removejob', 'Removes A Players Job (Admin Only)', { { name = 'id', help = 'Player ID' }, { name = 'job', help = 'Job name' } }, true, function(source, args)
    ...
    if Player then
        if Player.PlayerData.job.name == tostring(args[2]) then
            -- exports['Rc2-phone']:fireUser(tostring(args[2]), Player.PlayerData.citizenid)
            Wait(500)
...
QBCore.Commands.Add('company', 'Check Your company', {}, false, function(source)
    local src = source
    local PlayerCompany = QBCore.Functions.GetPlayer(src).PlayerData.company
    TriggerClientEvent('QBCore:Notify', src, Lang:t('info.company_info', {value = PlayerCompany.label, value2 = PlayerCompany.grade.name, value3 = PlayerCompany.onduty}))
end, 'user')

QBCore.Commands.Add('setcom', 'Set A Players Company (Admin Only)', { { name = 'id', help = 'Player ID' }, { name = 'company', help = 'company name' }, { name = 'grade', help = 'Grade' } }, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(tonumber(args[1]))
    if Player then
        Player.Functions.SetCompany(tostring(args[2]), tonumber(args[3]))
        TriggerEvent("Rc2-log:server:CreateLog", 
            "setjob",
            "Admin Set Company",
            "green",
            "**"..GetPlayerName(source) .. "** has gived company type : **"..tostring(args[2]).."** grade **"..tonumber(args[3]).."** to **"..GetPlayerName(Player.PlayerData.source).."**",
            false
        )
    else
...
QBCore.Commands.Add('ooc', Lang:t("command.ooc.help"), {}, false, function(source, args)
    ...
    TriggerEvent("Rc2-log:server:CreateLog", 
        ...
        "Player OOC",
        "green",
        "**"..GetPlayerName(source) .. "** Say : **"..message.."**",
        false
    )
end, 'user')
...
QBCore.Commands.Add('paycheck', 'Check your current paycheck', {}, false, function(source)
    ...
    TriggerClientEvent('QBCore:Notify', src, 'Your current paycheck $ '..paycheckData["amount"]..' Payment on Thursday or Friday', "success")
end)