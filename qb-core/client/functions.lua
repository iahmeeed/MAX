...
function QBCore.Functions.RequestModelHash(Model)
    RequestModel(Model)
...
function QBCore.Functions.AddProp(Name)
    if QBConfig.PropList[Name] ~= nil then
        if not HasProp then
          HasProp = true
          QBCore.Functions.RequestModelHash(QBConfig.PropList[Name]['prop'])
          local CurrentProp = CreateObject(QBConfig.PropList[Name]['hash'], 0, 0, 0, true, false, false)
          ...
          AttachEntityToEntity(CurrentProp, PlayerPedId(), GetPedBoneIndex(PlayerPedId(), QBConfig.PropList[Name]['bone-index']['bone']), QBConfig.PropList[Name]['bone-index']['X'], QBConfig.PropList[Name]['bone-index']['Y'], QBConfig.PropList[Name]['bone-index']['Z'], QBConfig.PropList[Name]['bone-index']['XR'], QBConfig.PropList[Name]['bone-index']['YR'], QBConfig.PropList[Name]['bone-index']['ZR'], true, true, false, true, 1, true)
...
function QBCore.Functions.RequestAnimationDict(AnimDict)
...
function QBCore.Functions.NetworkControl(entity)
...
function QBCore.Functions.RemoveProp()
...
function QBCore.Functions.GetPropStatus()
  return HasProp
end
...
--     TriggerEvent('notifications:sendNotification', color, text, 3000)
-- end
...
function QBCore.Functions.Notify(text, texttype, length)
    ...
    TriggerEvent("notifications:sendNotification", texttype, text, length)
end
...
function QBCore.Functions.SetVehicleProperties(vehicle, props)
    if DoesEntityExist(vehicle) then
		...
		TriggerServerEvent('Rc2-mechanic:server:loadStatus', props, VehToNet(vehicle))
    end
end
